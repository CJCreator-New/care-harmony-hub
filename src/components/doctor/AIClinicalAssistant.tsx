...
export const AIClinicalAssistant = ({ patientId }: { patientId: string }) => {
  ...
  const checkDrugInteractions = async (medications: string[]) => {
    try {
      const results = await clinicalDecisionSupport.checkDrugInteractions(medications, {
        patientId,
      });
      const mapped = results.map((r) => ({
        drugs: medications,
        severity: r.severity,
        mechanism: r.description,
      ...
      setDrugInteractions([
        {
          drugs: medications,
          severity: 'severe',
          mechanism: 'Clinical Decision Support check failed - pharmacist review required',
          recommendation:
            'Enforcing fail-closed safety invariant. Review medications with pharmacy.',
        },
      ]);
    }
  };
                    ...
                    <div>
                      ...
                      <ul className="text-sm text-muted-foreground space-y-1">
                        {suggestion.supportingFactors.map((evidence) => (
                          <li
                            key={`evidence-${suggestion.icd10Code}-${evidence}`}
                    ...
                    <div>
                      ...
                      <ul className="text-sm text-muted-foreground space-y-1">
                        {suggestion.supportingFactors.map((test) => (
                          <li
                            key={`test-${suggestion.icd10Code}-${test}`}
                            className="flex items-center"
                          >
