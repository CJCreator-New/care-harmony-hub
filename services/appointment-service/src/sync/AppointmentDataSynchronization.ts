...
export class AppointmentDataSynchronization {
  ...
  constructor(private fastify: FastifyInstance) {
    this.syncService = new AppointmentSyncService(fastify);
  ...
  async initialize(): Promise<void> {
    ...
    this.fastify.log.info('Appointment data synchronization initialized');
  }
  ...
  private async setupRoutes(): Promise<void> {
    ...
    fastify.post('/api/appointments/sync', async (request, reply) => {
      try {
    ...
    fastify.get('/api/appointments/sync/status', async (request, reply) => {
      try {
    ...
    fastify.post('/api/appointments/sync/conflicts/:conflictId/resolve', async (request, reply) => {
      try {
    ...
    fastify.get('/api/appointments/sync/conflicts', async (request, reply) => {
      try {
    ...
    fastify.post('/api/appointments/sync/validate', async (request, reply) => {
      try {
        const { appointments } = request.body as { appointments: any[] };
        const validationResult = await this.dataValidation.validateBatchAppointments(appointments);
    ...
    fastify.get('/api/appointments/sync/quality', async (request, reply) => {
      try {
    ...
    fastify.get('/api/appointments/sync/quarantine', async (request, reply) => {
      try {
    ...
    fastify.post('/api/appointments/sync/quarantine/:quarantineId/approve', async (request, reply) => {
      try {
    ...
    fastify.post('/api/appointments/sync/quarantine/:quarantineId/reject', async (request, reply) => {
      try {
        const { quarantineId } = request.params as { quarantineId: string };
    ...
    fastify.get('/api/appointments/sync/conflicts/stats', async (request, reply) => {
      try {
    ...
    fastify.post('/api/appointments/sync/conflicts/auto-resolve', async (request, reply) => {
      try {
  ...
  private async startBackgroundSync(): Promise<void> {
    ...
    setInterval(async () => {
      try {
        ...
        this.fastify.log.info('Background appointment sync completed');
      } catch (error) {
        this.fastify.log.error({ msg: 'Background appointment sync failed', error });
      }
    }, 5 * 60 * 1000); // 5 minutes
  ...
  async shutdown(): Promise<void> {
    ...
    this.fastify.log.info('Appointment data synchronization shut down');
  }
}