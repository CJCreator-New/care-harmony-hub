...
const requestSchema = z.object({
  ...
  action: z.enum(['initiate', 'approve', 'reject', 'cancel', 'discharge_ama']),
...
type DischargeWorkflowRow = {
  ...
  status: 'draft' | 'in_progress' | 'completed' | 'completed_ama' | 'cancelled';
...
const ACTION_TYPE_MAP = {
  ...
  discharge_ama: 'discharge_workflow_ama',
} as const;
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (payload.action === 'discharge_ama') {
      return await dischargeAMAWorkflow(supabase, actor, typedWorkflow, payload, corsHeaders);
    }
...
async function getActorContext(req: Request, supabase: ReturnType<typeof createClient>) {
  const { actor, response } = await getAuthorizedActor(req, [
    'doctor',
    'pharmacist',
    'receptionist',
    'nurse',
...
async function initiateWorkflow(
  ...
  if (!actor.roles.includes('doctor') && !actor.roles.includes('admin')) {
    return new Response(
      JSON.stringify({ error: 'Only doctors and admins can initiate discharge' }),
      {
        status: 403,
        headers: { 'Content-Type': 'application/json', ...corsHeaders },
      }
...
async function approveWorkflow(
  ...
  if (!(currentStep in NEXT_STEP)) {
    return new Response(
      JSON.stringify({ error: 'Workflow cannot be approved from its current state' }),
      {
        status: 400,
        headers: { 'Content-Type': 'application/json', ...corsHeaders },
      }
  ...
  if (rpcError || !updatedWorkflow) {
    ...
    if (directError || !directWorkflow) {
      return new Response(
        JSON.stringify({
          error:
            'Failed to advance discharge workflow — conflict detected or already updated by another user',
        }),
        {
          status: 409,
          headers: { 'Content-Type': 'application/json', ...corsHeaders },
...
async function rejectWorkflow(
  ...
  if (!(currentStep in PREVIOUS_STEP)) {
    return new Response(
      JSON.stringify({ error: 'Workflow cannot be rejected from its current state' }),
      {
        status: 400,
        headers: { 'Content-Type': 'application/json', ...corsHeaders },
      }
  ...
  // Clinical rejection aborts directly back to Step 1 (doctor).
  // Administrative rejection rolls back to the immediately preceding discipline (N - 1).
  const rejectionType = (payload.metadata?.rejectionType as string) || 'administrative';
  const isClinical = rejectionType === 'clinical';
  const previousStep = isClinical
    ? 'doctor'
    : PREVIOUS_STEP[currentStep as keyof typeof PREVIOUS_STEP];
  const now = new Date().toISOString();
  ...
  if (rpcError || !updatedWorkflow) {
    ...
    if (directError || !directWorkflow) {
      return new Response(
        JSON.stringify({
          error:
            'Failed to reject discharge workflow — conflict detected or already updated by another user',
        }),
        {
          status: 409,
          headers: { 'Content-Type': 'application/json', ...corsHeaders },
  ...
  const previousRole = isClinical
    ? 'doctor'
    : previousStep === 'billing'
      ? 'receptionist'
      : previousStep;
  await notifyRole(supabase, updatedWorkflow as DischargeWorkflowRow, previousRole, {
    title: isClinical
      ? 'Discharge workflow clinical hold / returned to Doctor'
      : 'Discharge workflow returned for correction',
    message: isClinical
      ? `Clinical instability / concern flagged at ${currentStep} step. Discharge aborted back to doctor. Reason: ${payload.reason}`
      : `The ${currentStep} step rejected discharge and returned it to ${previousStep}. Reason: ${payload.reason}`,
  });
...
async function dischargeAMAWorkflow(
  supabase: ReturnType<typeof createClient>,
  actor: { userId: string; hospitalId: string; fullName: string; roles: string[] },
  workflow: DischargeWorkflowRow,
  payload: RequestPayload,
  ...
  if (!actor.roles.includes('doctor') && !actor.roles.includes('admin')) {
    return new Response(
      JSON.stringify({
        error: 'Only attending physicians and administrators can execute AMA discharge',
      }),
      {
        status: 403,
        headers: { 'Content-Type': 'application/json', ...corsHeaders },
  ...
  if (!payload.reason || payload.reason.trim().length < 5) {
    return new Response(
      JSON.stringify({
        error: 'A clinical rationale of at least 5 characters is required for AMA discharge',
      }),
      {
        status: 400,
        headers: { 'Content-Type': 'application/json', ...corsHeaders },
  ...
  const nextMetadata = {
    ...
    ...(payload.metadata || {}),
    discharge_type: 'ama',
    ama_rationale: payload.reason.trim(),
    ama_timestamp: now,
    ama_authorized_by: actor.userId,
    ama_authorized_by_name: actor.fullName,
  };

  const { data: updatedWorkflow, error } = await supabase
    .from('discharge_workflows')
    .update({
      current_step: 'completed_ama',
      status: 'completed_ama',
      last_action_by: actor.userId,
      last_action_at: now,
  ...
  if (error || !updatedWorkflow) {
    return new Response(
      JSON.stringify({
        error:
          'Failed to execute AMA discharge — conflict detected or already updated by another user',
      }),
      {
        status: 409,
        headers: { 'Content-Type': 'application/json', ...corsHeaders },
  ...
  await writeAuditTrail(supabase, updatedWorkflow as DischargeWorkflowRow, actor, {
    transitionAction: 'discharge_ama',
    fromStep: currentStep,
    toStep: 'completed_ama',
  ...
  // Post-facto billing audit notification for AMA discharge
  await notifyRole(supabase, updatedWorkflow as DischargeWorkflowRow, 'receptionist', {
    title: 'Patient Discharged Against Medical Advice (AMA)',
    message: `Patient was discharged AMA by ${actor.fullName}. Post-facto billing audit required.`,
  });
...
async function cancelWorkflow(
  ...
  const isInitiatingDoctor =
    actor.roles.includes('doctor') && workflow.initiated_by === actor.userId;
  if (!isAdmin && !isInitiatingDoctor) {
    return new Response(
      JSON.stringify({
        error: 'Only the initiating doctor or an admin can cancel this discharge workflow',
      }),
      {
        status: 403,
        headers: { 'Content-Type': 'application/json', ...corsHeaders },
  ...
  if (error || !updatedWorkflow) {
    return new Response(
      JSON.stringify({
        error:
          'Failed to cancel discharge workflow — conflict detected or already updated by another user',
      }),
      {
        status: 409,
        headers: { 'Content-Type': 'application/json', ...corsHeaders },
...
async function writeAuditTrail(
  ...
  actor: { userId: string; hospitalId: string; fullName: string; roles: string[] },
  transition: {
    transitionAction: 'initiate' | 'approve' | 'reject' | 'cancel' | 'discharge_ama';
  ...
  const actorRole =
    actor.roles.find((role) =>
      (
        STEP_ROLE_MAP[transition.toStep as keyof typeof STEP_ROLE_MAP] ??
        STEP_ROLE_MAP[transition.fromStep as keyof typeof STEP_ROLE_MAP] ??
        []
