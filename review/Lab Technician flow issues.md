# Lab Technician Flow Issues - Verification Status
...
- `npx playwright test tests/e2e/laboratory.spec.ts tests/e2e/pharmacy.spec.ts tests/e2e/doctor-workflow.spec.ts --project=chromium --workers=1` : blocked by outdated login route assumptions in tests (`/login`), not by laboratory runtime failures.
