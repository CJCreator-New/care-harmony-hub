...
async function loginAs(page: Page, userKey: keyof typeof TEST_USERS): Promise<void> {
  ...
  try {
    ...
    await page
      .waitForURL(new RegExp('/(dashboard|patient/portal|portal|home)', 'i'), { timeout: 30000 })
      .catch(() => {});
    await page.waitForLoadState('networkidle');
  } catch (error) {
...
async function isRouteBlocked(
  ...
  const accessDenied = await page
    .locator(
      ':text("Access Denied"), :text("access denied"), :text("Unauthorized"), :text("Permission denied")'
    )
    .first()
...
test.describe('HIMS E2E Diagnostic Suite', () => {
    ...
    test('1.2: Pharmacist cannot access /consultations', async ({ page }) => {
      ...
      if (!blocked) {
        logIssue(
          'CRITICAL',
          'RBAC',
          'Pharmacist can access restricted route',
    ...
    test('1.3: Patient cannot access /patients (management)', async ({ page }) => {
      ...
      if (!blocked) {
        logIssue(
          'CRITICAL',
          'RBAC',
          'Patient can access patient management',
    ...
    test('1.5: Receptionist cannot access /settings (admin only)', async ({ page }) => {
      ...
      if (!blocked) {
        logIssue(
          'CRITICAL',
          'RBAC',
          'Receptionist can access admin settings',
        ...
        for (const roleKey of roles) {
          ...
          if (shouldHaveAccess && blocked) {
            ...
            logIssue(
              'HIGH',
              'RBAC',
              `${roleKey} denied legitimate access`,
            ...
            logIssue(
              'CRITICAL',
              'RBAC',
              `${roleKey} unauthorized access to ${route}`,
    ...
    test('2.1: Doctor can create prescription', async ({ page }) => {
      ...
      if (!success) {
        logIssue(
          'HIGH',
          'Workflow',
          'Doctor cannot create prescription',
    ...
    test('2.2: Pharmacist can approve prescription', async ({ page }) => {
      ...
      if (!success) {
        logIssue(
          'HIGH',
          'Workflow',
          'Pharmacist cannot approve prescriptions',
      ...
      if (!blocked || inQueue) {
        ...
        if (!hasDispense) {
          logIssue(
            'MEDIUM',
            'Workflow',
            'Nurse cannot dispense from queue',
    ...
    test('2.4: Patient can view prescriptions (portal)', async ({ page }) => {
      ...
      if (!success) {
        logIssue(
          'MEDIUM',
          'Workflow',
          'Patient portal missing prescriptions',
    ...
    test('2.5: Lab order workflow accessible', async ({ page }) => {
      ...
      if (!success) {
        logIssue(
          'MEDIUM',
          'Workflow',
          'Doctor cannot create lab orders',
      ...
      if (blocked) {
        ...
        if (patientsBlocked) {
          logIssue(
            'MEDIUM',
            'Workflow',
            'Nurse cannot access patient data',
    ...
    test('3.2: Pharmacist data not visible to receptionist', async ({ page }) => {
      ...
      if (!blocked) {
        logIssue(
          'CRITICAL',
          'Data Isolation',
          'Receptionist can access pharmacy',
    ...
    test('3.3: Patient data isolated from other patients', async ({ page }) => {
      ...
      if (blocked) {
        logIssue(
          'CRITICAL',
          'Data Isolation',
          'Patient cannot access own portal',
    ...
    test('3.4: Lab results visible to ordering doctor', async ({ page }) => {
      ...
      if (blocked) {
        logIssue(
          'HIGH',
          'Workflow',
          'Doctor cannot see lab orders/results',
  ...
  test.describe('Suite 4: Permission Enforcement at Action Level', () => {
      ...
      if (!blocked) {
        ...
        if (hasCreate) {
          logIssue(
            'MEDIUM',
            'RBAC',
            'Receptionist has create patient button',
    ...
    test('4.2: Pharmacist cannot edit prescription (only approve)', async ({ page }) => {
      ...
      const editBtn = page
        .locator('button:has-text("Edit"), button:has-text("Modify"), button:has-text("Update")')
        .first();
      ...
      if (hasEdit) {
        logIssue(
          'HIGH',
          'RBAC',
          'Pharmacist can edit prescription',
    ...
    test('4.3: Nurse cannot delete orders', async ({ page }) => {
      ...
      for (const route of routes) {
        ...
        const deleteBtn = page
          .locator('button:has-text("Delete"), button:has-text("Remove"), [aria-label*="delete" i]')
          .first();
  ...
  test.describe('Suite 5: Error Handling & Resilience', () => {
    ...
    test('5.3: Session expiry handled gracefully', async ({ page }) => {
      ...
      const hasLoginPrompt = await page
        .locator(':text("login"), :text("sign in"), :text("session")')
        .first()
      ...
      if (!hasLoginPrompt) {
        logIssue(
          'MEDIUM',
          'UX',
          'Session expiry not clearly communicated',
