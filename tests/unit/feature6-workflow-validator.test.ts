  ...
  supabase: {
    from: vi.fn((table: string) => ({
      insert: vi.fn((rows: any[]) => {
        if (table === 'workflow_executions' && rows && rows[0]) {
          executions.set(rows[0].workflow_id, rows[0]);
        } else if (table === 'workflow_audit_logs' && rows && rows[0]) {
          const list = auditLogs.get(rows[0].workflow_id) || [];
          list.push(...rows);
          auditLogs.set(rows[0].workflow_id, list);
        }
        return Promise.resolve({ data: rows, error: null });
      }),
      update: vi.fn((updates: any) => ({
        eq: vi.fn((col: string, val: string) => {
          const existing = executions.get(val) || {};
          executions.set(val, { ...existing, ...updates });
      ...
      select: vi.fn(() => ({
        eq: vi.fn((col: string, val: string) => {
          ...
          return Object.assign(Promise.resolve({ data: rows, error: null }), {
            single: vi.fn(() =>
              Promise.resolve({ data: executions.get(val) || null, error: null })
            ),
          });
        }),
      })),
...
describe('Feature 6: Workflow Validator', () => {
  ...
  describe('validateWorkflowCompliance', () => {
    ...
    it('6.15: Validates compliant workflow', async () => {
      // Act
      const compliance = await workflowValidator.validateWorkflowCompliance(compliantWorkflowId);
  ...
  describe('Multi-Role Authorization', () => {
    ...
    it('6.23: Nurse can only verify prescriptions & add observations', async () => {
      ...
      await expect(
        workflowValidator.initializeWorkflow('prescription', mockHospitalId, mockNurseId, 'nurse', {
          patient_id: 'p1',
          medication: 'm1',
          dosage: 'd1',
          frequency: 'f1',
    ...
    it('6.25: Billing staff can only generate/submit claims', async () => {
      ...
      await expect(
        workflowValidator.initializeWorkflow('billing', mockHospitalId, mockBillingId, 'billing', {
          appointment_id: 'a1',
          procedures: ['99213'],
          diagnoses: ['J06'],
        })
  ...
  describe('Workflow Timeout Handling', () => {
    it('6.26: Tracks timeouts for each workflow step', async () => {
      ...
      const doctorStep = workflows.find((s) => s.step_id === 'appointment.doctor_confirm');
      const nurseStep = workflows.find((s) => s.step_id === 'appointment.nurse_confirm');
    ...
    it('6.27: Prescription workflow has short timeouts', async () => {
      ...
      const createStep = workflows.find((s) => s.step_id === 'prescription.create');
      const verifyStep = workflows.find((s) => s.step_id === 'prescription.pharmacist_verify');
  ...
  describe('Workflow Step Sequencing', () => {
    it('6.28: Appointment workflow follows correct sequence', async () => {
      ...
      expect(workflows[1].next_steps).toContain('appointment.nurse_confirm');
    });
