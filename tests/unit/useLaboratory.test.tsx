...
// Prevent auto-fetch useEffects from running during tests
...
describe('useLabResults', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    // Default mock for the auto-fetch useEffect
    mockSupabaseClient.from.mockReturnValue({
      ...mockSupabaseClient.from(),
      select: vi.fn().mockReturnThis(),
      eq: vi.fn().mockReturnThis(),
  ...
  it('adds a lab result successfully', async () => {
    ...
    await act(async () => {
      await result.current.addLabResult({
...
describe('useCriticalValueNotifications', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    // Default mock for auto-fetch and channel subscription
    mockSupabaseClient.from.mockReturnValue({
      ...mockSupabaseClient.from(),
      select: vi.fn().mockReturnThis(),
      eq: vi.fn().mockReturnThis(),
